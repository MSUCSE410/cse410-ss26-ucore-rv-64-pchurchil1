...
struct inode *ialloc(uint dev, short type)
{
	...
	for (inum = 1; inum < sb.ninodes; inum++) {
		...
		if (dip->type == 0) { // a free inode
			...
			dip->type = type;
			dip->nlink = 1; // ADDED: new inode starts with one hard link
			bwrite(bp);
			brelse(bp);
...
void iupdate(struct inode *ip)
{
	...
	dip->nlink = ip->nlink; // ADDED: update link count
...
void ivalid(struct inode *ip)
{
	...
	if (ip->valid == 0) {
		...
		ip->nlink = dip->nlink; // Link count added
...
void iput(struct inode *ip)
{
	...
	// Uncommented && ip->nlink == 0 to ensure we completely check the inode can be freed
...
// ADDED: remove a directory entry by name from directory dp
// Returns 0 on success
// Directory-side remove operation needed by unlinkat
